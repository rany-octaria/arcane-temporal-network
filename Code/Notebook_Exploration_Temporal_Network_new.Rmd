---
title: "Exploratory Analysis of the French Hospital Transfer Network (2024)"
...
> **Before running:** Open this project from the ARCANE-Temporal-Network RStudio
> project. Download the transfer datasets from the shared SharePoint folder and
> place them in `Datasets/MCO_SSR_HBN_2024/`.
...
dir.create(here("Outputs", "Network"), recursive = TRUE, showWarnings = FALSE)
...
weekly_plots <- map(metrics_to_plot, function(m) {
  ...
  ggsave(here("Outputs", "Plots", paste0("weekly_", m, ".png")),
         p, width = 13, height = 7, dpi = 200)
  p
})

# Print all
walk(weekly_plots, print)
```
...
ggsave(here("Outputs", "Plots", "weekly_transfers_by_type.png"),
...
ggsave(here("Outputs", "Plots", "weekly_senders_by_type.png"),
...
## 3.1 Compute metrics per time window
...
rds_weekly  <- here("Outputs", "network_metrics_weekly.rds")
rds_monthly <- here("Outputs", "network_metrics_monthly.rds")
...
walk(key_metrics, function(m) {
  ...
  print(p)
  ggsave(here("Outputs", "Plots", paste0("network_weekly_", m, ".png")),
         p, width = 13, height = 7, dpi = 200)
})
```
...
edges_plot <- edges_geo %>%
  ...
  mutate(weight_scaled = scales::rescale(log1p(total_weight), to = c(0.1, 1.5)))
...
ggsave(here("Outputs", "Network", "network_by_hospital_type.png"),
       p_network_by_type, width = 14, height = 10, dpi = 250)
```

---

# 5. Network Analysis by Hospital Type
...
knitr::kable(
  hosp_type_stats,
  caption = "Hospital characteristics by type",
  col.names = c("Type", "N hospitals", "Median MCO beds",
                "Mean MCO beds", "With coordinates"),
  digits = 1
)
...
print(p_type_counts)
ggsave(here("Outputs", "Plots", "hospital_counts_by_type.png"),
...
ggsave(here("Outputs", "Plots", "outstrength_by_type.png"),
...
print(p_type_flow)
ggsave(here("Outputs", "Plots", "flow_between_types_heatmap.png"),
